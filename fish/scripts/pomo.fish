...
# Taken from https://github.com/joncinque/pomo.fish/blob/master/pomo.fish
...
function start_timer -d "Start a new timer"
  ...
  echo '{"pomodoroRunning": true}' | nc -U /tmp/statusbar 
  ...
  if test $stopped = '0'
    echo '{"pomodoroRunning": false}' | nc -U /tmp/statusbar 
    if test $notify = '1'
      # switch (uname)
      #   case Linux FreeBSD NetBSD DragonFly
